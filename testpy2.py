    ...
    if ids is None:
        ...
        for i in range(0, len((bboxs))):
            ...
            if (x2 == x1):
                ...
                temp = ((y1-y2)/(x2-x1))
                angle =-180+(1* int(math.degrees(math.atan(temp))))

            elif (x1 < x2 and y1 < y2):
                temp = ((y2-y1)/(x2-x1))
                angle = 180-(int(math.degrees(math.atan(temp))))

            elif (x1 > x2 and y1 < y2):
                temp = ((y2-y1)/(x1-x2))
                angle =180- (int(180-math.degrees(math.atan(temp))))

            elif (x1 > x2 and y1 > y2):
                temp = ((y1-y2)/(x1-x2))
                angle = -180+(1* int(180-(math.degrees(math.atan(temp)))))
    return centres
#####################################################################################
...
#################################  ADD YOUR CODE HERE  ###############################
    ...
    if(len(centre5)):
        ...
        roundang()
        print('actual ',angle)
        sim.setObjectOrientation(handle,-1,[0,0,angle])
...
#################################  ADD YOUR CODE HERE  ################################
    ...
    while(True):
        ...
        if(len(frame)):
            frame1=cv2.resize(frame[0],(512,512),interpolation=cv2.INTER_CUBIC)
